...
class stationeryTabview extends StatelessWidget {
  final CartController cartController =
      Get.put(CartController()); // Ensure CartController is available

  stationeryTabview({Key? key}) : super(key: key);

  final List<Map<String, String>> stationery = [
    {
      'id': '25',
      'title': 'Coca Cola',
      'imageUrl':
        ...
        itemBuilder: (ctx, i) => GestureDetector(
          onTap: () {
            cartController.addItem(
              stationery[i]['id']!,
              stationery[i]['title']!,
              stationery[i]['imageUrl']!,
              double.parse(stationery[i]
                  ['price']!), // Convert the price string to double
              stationery[i]['Brandname']!,
              stationery[i]['Description']!,
              stationery[i]['Quantity']!,
            );
          },
          child: GridTile(
            child: Image.network(stationery[i]['imageUrl']!, fit: BoxFit.cover),
            footer: GridTileBar(
