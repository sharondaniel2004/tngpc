...
class householdcareTabview extends StatelessWidget {
  final CartController cartController =
      Get.put(CartController()); // Ensure CartController is available

  householdcareTabview({Key? key}) : super(key: key);

  final List<Map<String, String>> householdcare = [
    {
      'id': '9',
      'title': 'Coca Cola',
      'imageUrl':
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: GridView.builder(
        ...
        itemCount: householdcare.length,
        ...
        itemBuilder: (ctx, i) => GestureDetector(
          onTap: () {
            cartController.addItem(
                householdcare[i]['id']!,
                householdcare[i]['title']!,
                householdcare[i]['imageUrl']!,
                double.parse(householdcare[i]
                    ['price']!), // Convert the price string to double
                householdcare[i]['Brandname']!,
                householdcare[i]['Description']!,
                householdcare[i]['Quantity']!);
          },
          child: GridTile(
            child:
                Image.network(householdcare[i]['imageUrl']!, fit: BoxFit.cover),
            footer: GridTileBar(
